...
class Filter:
    def __init__(self, title, tagStr):
        self.name = title
...
def save():
    with open('history.csv', 'w', newline='') as csvfile:
        ...
        for f in filters:
            writer.writerow(["f", f.name, "#".join(f.tags)])
...
def isRelevantMatch(match):
    relevantPlayers = []

    for filter in filters:
        if currentFilter in filter.tags:
            relevantPlayers.append(filter.name)
    
    return match.p1 in relevantPlayers and match.p2 in relevantPlayers
...
def run():
    ...
    for i in range(len(tournaments)):
        ...
        relevantMatches = tournament.matches if currentFilter == "" else list(filter(isRelevantMatch, tournament.matches))
        
        for m in relevantMatches:
            shouldUseMedian = tournament.id == 0 or currentFilter != ""
            entryPoint = ENTRY if shouldUseMedian else getMedian()
            ...
            if m.p1 not in players:
                players.append(m.p1)
                rankings.append((m.p1, entryPoint))

            if m.p2 not in players:
                players.append(m.p2)
                rankings.append((m.p2, entryPoint))
        updates = []
...
def rec(player, stat):
    ...
    for t in tournaments:
        relevantMatches = t.matches if currentFilter == "" else list(filter(isRelevantMatch, t.matches))
...
def set_filter():
    ...
    currentFilter = str(input("Filter >"))
    historic.clear()
    run()
    ...
    with open('history.csv', newline='') as csvfile:
        ...
        for row in reader:
            if row[0] == "t":
                ...
                filters.append(Filter(row[1], row[2]))
            elif row[0] == "m":
...
def filters_loop():
    global filters_index, filters_items
    menu_loop(filters_index, filters_items, draw_filters)

def draw_menu():
    print("/=======================\\")
    print("| Darts Rankings System |")
    print("|-----------------------|")
    print("| By Kyle Mandell, 2025 |")
    print("\\=======================/")
    print("This software should not be considered stable. You should regularly back-up history.csv to avoid data loss.")
    print('''Copyright 2025 Kyle Mandell
